  ...
  return (
    ...
    >
        ...
        <div className="flex touch-pan-y">
          {activeSlides.map((slide, index) => (
            <div className="relative min-w-0 flex-[0_0_100%]" key={`${slide.linkHref}-${index}`}>
              <div className="relative grid aspect-[16/9] min-h-[360px] w-full overflow-hidden bg-[#f5f7fa] sm:aspect-[16/7] sm:min-h-0 sm:grid-cols-[48%_52%]">
                ...
                <div className="relative z-[1] flex flex-col justify-center px-8 pb-20 pt-10 sm:px-12 lg:px-[max(3rem,calc((100vw-80rem)/2))]">
                  <span className="mb-3 w-fit border-l-4 border-secondary pl-3 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-600">
                    Destaque ARKNET
                  </span>
                  <h2 className={`max-w-md text-2xl font-black leading-[1.05] tracking-tight text-slate-900 transition duration-700 sm:text-4xl lg:text-5xl ${selectedIndex === index ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-70'}`}>
                    {slide.title || 'Tecnologia para o seu próximo passo'}
                  </h2>
                  <p className={`mt-4 max-w-sm text-sm leading-relaxed text-slate-600 transition delay-100 duration-700 sm:text-base ${selectedIndex === index ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-60'}`}>
                    {slide.subtitle || 'Encontre equipamentos selecionados e soluções para trabalhar, estudar e estar sempre conectado.'}
                  </p>
                  <span className="mt-6 w-fit bg-secondary px-5 py-2.5 text-xs font-bold uppercase tracking-wide text-white shadow-[0_10px_24px_rgba(227,6,19,0.2)] transition group-hover:bg-primary group-hover:text-white">
                    Ver produto
                  </span>
                </div>
                ...
                {selectedIndex === index && !reducedMotion && (
                  <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[2] h-1 bg-white/20">
      ...
      {activeSlides.length > 1 && (
        <>
          <button type="button" onClick={() => { stopAutoplay(); scrollPrev() }} aria-label="Banner anterior" className="absolute left-4 top-1/2 z-10 -translate-y-1/2 border border-white/30 bg-slate-950/40 p-2.5 text-white backdrop-blur-sm transition hover:bg-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white sm:left-8">
            <ChevronLeft className="h-5 w-5" aria-hidden="true" />
          </button>
          <button type="button" onClick={() => { stopAutoplay(); scrollNext() }} aria-label="Próximo banner" className="absolute right-4 top-1/2 z-10 -translate-y-1/2 border border-white/30 bg-slate-950/40 p-2.5 text-white backdrop-blur-sm transition hover:bg-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white sm:right-8">
